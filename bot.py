...
class MyClient(discord.Client):
	...
	def get_uptime(self):
		...
		if seconds % 60 > 0:
			time_string += " and "
			second_value = (seconds % 3600) % 60
			if (second_value) > 1:
				time_string += str(second_value)
				time_string += " seconds"
			else:
				time_string += str(second_value)
				time_string += " second"
	...
	async def on_ready(self):
		...
		logger.info(self.user.name + " is ready (finished on_ready). Took "+self.get_uptime()+" to start")  # Event log
	...
	async def on_message(self, message):
		...
		if message.content.startswith(prefix + "get rank"):
			...
			if str(message.author.id) in self.data["servers"][str(guild.id)]["ranks"]:
				...
				percentage = int(
					round((self.data["servers"][str(guild.id)]["ranks"][str(message.author.id)] ** 0.5) % 1, 2) * 100)
			else:
				rank = 0
				percentage = 0
		...
		if guild.get_role(self.data["servers"][str(message.guild.id)]["roles"]["admin role id"]) in guild.get_member(
			...
			if message.content == prefix + "rules":
				...
				# !!! This is messy
				embed_welcome = discord.Embed(title="👋 Welcome to " + message.guild.name + ".",
											  description="[Discord community server description]\n\nTake a moment to familiarise yourself with the rules below.\nChannel <#831953098800889896> is for this, and <#610595467444879421> is for that.",
											  color=0xffc000)
				await message.channel.send(embed=embed_welcome)

				# Create the rules embed
				embed_rules = discord.Embed(title=self.data["servers"][str(guild.id)]["rules"]["title"],
											description=self.data["servers"][str(guild.id)]["rules"]["description"],
											color=0xffc000, inline=False)
				embed_rules.set_footer(text="Rules updated: • " + date.today().strftime("%d/%m/%Y"),
									   icon_url=guild.icon_url)
				embed_rules.add_field(name="Rules",
									  value="\n".join(self.data["servers"][str(guild.id)]["rules"]["list"]),
									  inline=True)
		...
		if message.author.id in self.data["config"]["developers"]:
			...
			if message.content.startswith(prefix + "kill"):
				...
				await message.channel.send(
					self.user.name + " shutting down after " + self.get_uptime() + " of uptime.\n" + argument)
				await client.close()
	...
	async def on_raw_reaction_add(self, payload: discord.RawReactionActionEvent):
		...
		for category in self.data["servers"][str(guild.id)]["roles"]["category list"]:
			if payload.message_id == self.data["servers"][str(payload.guild_id)]["roles"]["category list"][category][
				"message id"]:
		...
		for category in self.data["servers"][str(guild.id)]["roles"]["category list"]:  # For category in list
			for role in self.data["servers"][str(guild.id)]["roles"]["category list"][category][
				"role list"]:  # For role in category
				if self.data["servers"][str(guild.id)]["roles"]["category list"][category]["role list"][role][
		...
		# If we want to do something in case of errors we'd do it here.
			...
			logger.error(
				"Exception: discord.HTTPException. Could not add role " + role.name + " to " + payload.member.name)  # Event log
	...
	async def on_raw_reaction_remove(self, payload: discord.RawReactionActionEvent):
		...
		# If we want to do something in case of errors we'd do it here.
			...
			logger.error("Exception: discord.HTTPException. Could not remove role " + role.name + " from ",
						 member.name)  # Event log
