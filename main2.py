...
if __name__ == "__main__":
	...
	# Help command
	@slash.slash(name="help", description="Displays help information.", options=[
		create_option(name="command", description="The slash command you want help with. Leave blank if you want to see help for all commands.", option_type=3, required=False)], guild_ids=guild_ids)
	...
	# Embed command
	@slash.slash(name="embed", description="Sends a custom made embed.", options=[create_option(name="title", description="The embed title.", option_type=3, required=True),
																				  create_option(name="description", description="The embed description.", option_type=3, required=True),
																				  create_option(name="color", description="The embed color. Takes hexadecimal colour values.", option_type=3, required=False)], guild_ids=guild_ids)
	...
	async def _roles(ctx):
			...
			for category in data["servers"][str(guild.id)]["roles"]["categories"]:  # For category in roles
				...
				for role in data["servers"][str(guild.id)]["roles"]["categories"][category]["list"]:  # For role in category
					...
					if fill < 5:
						...
						fill += 1
						print(data["servers"][str(guild.id)]["roles"]["categories"][category]["list"][role]["name"])
					else:
				...
				category_message = await ctx.channel.send(content="Click button to get role", components=rows)
		...
		# If the roles haven't been set up
			...
			logger.debug("Roles have not been set up for " + str(ctx.guild.id))  # Event log
